...
class Parser:
    ...
    def func_def(self):
        ...
        if self.current_tok.type == TT_IDENTIFIER:
            ...
            if self.current_tok.type != TT_LPAREN:  # Fixed this line
                return res.failure(InvalidSyntaxError(
                    self.current_tok.pos_start, self.current_tok.pos_end,
                    f"Expected '('"
                ))
            ...
            var_name_tok = None
            if self.current_tok.type != TT_LPAREN:  # And this line
                return res.failure(InvalidSyntaxError(
                    self.current_tok.pos_start, self.current_tok.pos_end,
...
BuiltInFunction.input         = BuiltInFunction("print")
BuiltInFunction.input_int     = BuiltInFunction("print")
...
class Interpreter:
    ...
    def visit_CallNode(self, node, context):
        ...
        # Handle case where arg_names might not exist
        expected_arg_count = len(getattr(value_to_call, 'arg_names', []))
        if len(args) != expected_arg_count:
            return res.failure(RTError(
                node.pos_start, node.pos_end,
                f"{abs(len(args) - expected_arg_count)} too {'many' if len(args) > expected_arg_count else 'few'} args passed into '{value_to_call.name}'",
                context
            ))
