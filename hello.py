...
TT_EOF         = 'EOF'  # To detect if we reach EOF in Parser
TT_NEWLINE     = 'NEWLINE'
    ...
    def make_tokens(self):
        ...
        while self.current_char != None:
            if self.current_char in ' \t':
                ...
                tokens.append(self.make_minu_or_arrow())
    ...
    def make_string(self):
        ...
        while self.current_char != None and (self.current_char != '"' or escape_character):
            if escape_character:
                ...
                escape_character = False  # Add this line
            else:
                if self.current_char == '\\':
...
class ParseResult:
    def __init__(self):
        self.error = None
        self.node = None
        self.last_registered_advance_count = 0  # Add this
        self.advance_count = 0
        self.to_reverse_count = 0

    def register_advancement(self):
        self.last_registered_advance_count = 1  # Add this
        self.advance_count += 1

    def register(self, res):
        self.last_registered_advance_count = res.advance_count  # Add this
    ...
    def failure(self, error):
        if not self.error or self.last_registered_advance_count == 0:  # Modified this condition
            self.error = error
        return self
...
class Parser:
    ...
    def statements(self):
        ...
        # Skip initial newlines/semicolons
        ...
        # Parse additional statements separated by newlines/semicolons
        while True:
            newline_count = 0
            while self.current_tok.type == TT_NEWLINE:
                res.register_advancement()
            ...
            if not statement:
                # No more statements, reverse any advances
        ...
        if self.current_tok.type == TT_LPAREN:
            ...
            if self.current_tok.type == TT_RPAREN:
                ...
                if res.error:
                    return res.failure(InvalidSyntaxError(
                        self.current_tok.pos_start, self.current_tok.pos_end,
                        "Expected ')', 'initiate', 'IF', 'FOR', 'WHILE', 'FUN', int, float, identifier, '+', '-', '(', '[' or 'NOT'"
                    ))
...
class List(Value):
    ...
    def multed_by(self, other):
        if isinstance(other, List):
            new_list = self.copy()
            new_list.elements.extend(other.elements) # extend() allows for concatination
...
class BuiltInFunction(BaseFunction):
    ...
    def execute(self, args):
        ...
        # Set arg_names from the method
        self.arg_names = getattr(method, 'arg_names', [])
    ...
    def __repr__(self):
        return f"<buil-in function {self.name}>"
    ...
    def execute_pop(self, exec_ctx):
        ...
        if not isinstance(list_, List):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                "First argumentmust must be list",
                exec_ctx
            ))
        ...
        if not isinstance(index, Number):
            return RTResult().failure(RTError(
                self.pos_start, self.pos_end,
                "Second argumentmust must be number",
                exec_ctx
            ))
...
#######################################
# CONSTANT FOR ALL BUILT-IN-FUNCTIONS
#######################################
...
class Interpreter:
    ...
    def visit_UnaryOpNode(self, node, context):
        ...
        if node.op_tok.type == TT_MINUS:  # Changed from .value to .type
...
def run(fn, text):
    ...
    context.symbol_table = global_symbol_table  # Set instance attribute, not class attribute
